...
export default class App extends Component {
  ...
  async componentDidMount() {
    ...
    */
    Task2Manager.doTaskX('Learn "React Native call Swift"', 90)
    /*
    Task2Manager.getAllTasks({
      name: "Hoang",
      email: "sunlight4d@gmail.com"
    }, (error, tasks) => {
      if (error) {
        alert(`getAllTasks: ${JSON.stringify(error)}`)
      } else {
        alert(`getAllTasks: ${JSON.stringify(tasks)}`)
      }
    })
    */
   //Promise
   /*
    Task2Manager.getSomeTasks("others").then((tasks) => {
      //then = "resolve"
      alert(`getSomeTasks = ${JSON.stringify(tasks)}`)
    }).catch((error) => {
      //catch = "reject"      
    ...
    */
    //async - await
    try {
      let tasks = await Task2Manager.getSomeTasks("all") //await = "wait this line finished"
      alert(`getSomeTasks2 = ${JSON.stringify(tasks)}`)
    } catch (error) {
      alert(`Error's code: ${error.code}, userInfo = ${JSON.stringify(error.userInfo)}`)
    }
  }
  render() {
    return (
      <View style={styles.container}>
        <Text style={styles.welcome}>React Native calls Swift</Text>
